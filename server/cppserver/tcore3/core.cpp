...
namespace tcore {
    ...
    bool core::launchUdpSession(iUdpSession * session, const char * ip, const int32 port) {
    ...
    iHttpRequest * core::getHttpRequest(const uint64 delivery, const int64 id, const char * url, iHttpResponse * response, const iContext & context) {
        return httper::getInstance()->getHttpRequest(delivery, id, url, response, context);
    }

    void core::startTimer(iTimer * timer, const int32 id, int64 delay, int32 count, int64 interval, const iContext context, const char * file, const int32 line){
        timermgr::getInstance()->startTimer(timer, id, context, delay, count, interval, file, line);
    }

    void core::killTimer(iTimer * timer, const int32 id, const iContext context){
        timermgr::getInstance()->killTimer(timer, id, context);
    }

    void core::pauseTimer(iTimer * timer, const int32 id, const iContext context){
        timermgr::getInstance()->pauseTimer(timer, id, context);
    }

    void core::resumeTimer(iTimer * timer, const int32 id, const iContext context){
    ...
    void core::connectRedis(const char * ip, const int32 port, const char * passwd, const fConnectRedisCall call, const iContext context) {
    ...
    void core::logSync(const int64 tick, const char * log, const bool echo) {
        logger::getInstance()->logSync(tick, log, echo);
    }

    void core::logAsync(const int64 tick, const char * log, const bool echo) {
