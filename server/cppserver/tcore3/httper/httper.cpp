...
namespace tcore {
    ...
    static int32 s_current_thread_count;
    ...
    void httper::run() {
        ...
        while (true) {
            ...
            while (s_request_queue[index].tryPull(request)) {
                ...
                int32 code = curl_easy_perform(handle);
    ...
    httpRequest * httper::getHttpRequest(const uint64 delivery, const int64 id, const char * url, iHttpResponse * response, const iContext & context) {
    ...
    void httper::push(httpRequest * request) {
        ...
        if (s_current_thread_count != 0) {
            s_request_queue[::abs((int64)(request->_delivery) % s_current_thread_count)].push(request);
        }
    }

    void httper::update() {
        int64 tick = tools::time::getMicrosecond();
        httpRequest * request;
        while (s_response_queue.tryPull(request)) {
            request->onResponse();
            request->release();
