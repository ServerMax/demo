...
namespace tcore {
    ...
    public:
        const uint64 _delivery;

        static httpRequest * create(const uint64 delivery, const int32 id, const char * url, iHttpResponse * response, const iContext & context);
        void release();

        const char * getParams() { return _params.c_str(); }
        const char * getUrl() { return _url.c_str(); }
        void setError(const int32 error) { _error = error; }
        void onResponse();
    ...
    private:
        httpRequest(const uint64 delivery, const int32 id, const char * url, iHttpResponse * response, const iContext & context)
