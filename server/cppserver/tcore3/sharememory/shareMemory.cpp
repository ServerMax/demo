...
namespace tcore {
    shareMemory * shareMemory::create(const std::string & name, int size, bool exists) {
        ...
        const tools::memery::shareMemeryInfo * info = tools::memery::getShareMemory(path, name, size + sizeof(int32) + sizeof(tlib::spinlock));
        if (nullptr == info) {
            error(core::getInstance(), "getShareMemory error");
            return nullptr;
        }

        void * lockaddr = (char *)info->_vm + sizeof(int32);
