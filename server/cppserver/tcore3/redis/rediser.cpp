...
namespace tcore {
    ...
    rediser::rediser(const char * ip, const int32 port, const char * passwd) : _addr(ip, port), _passwd(passwd), _context(nullptr) {
    ...
    void rediser::completerSet(const uint32 id, const char * key, iRedisCompleter * completer, const void * data, const uint32 len, bool sync, const iContext context) {
        if (sync) {
    ...
    void rediser::completerGet(const uint32 id, const char * key, iRedisCompleter * completer, bool sync, const iContext context) {
        if (sync) {
            autolocker(lock, _lock);
            checkContext();
    ...
    void rediser::completerDel(const uint32 id, const char * key, iRedisCompleter * completer, bool sync, const iContext context) {
        if (sync) {
            autolocker(lock, _lock);
            checkContext();
    ...
    void rediser::callerSet(const uint32 id, const char * key, const fRedisCall & call, const void * data, const uint32 len, bool sync, const iContext context) {
        if (sync) {
            autolocker(lock, _lock);
            checkContext();
            redisReply * res = (redisReply *)redisCommand(_context, "set %s %b", key, data, (size_t)len);
    ...
    void rediser::callerGet(const uint32 id, const char * key, const fRedisCall& call, bool sync, const iContext context) {
        if (sync) {
            autolocker(lock, _lock);
            checkContext();
            redisReply * res = (redisReply *)redisCommand(_context, "get %s", key);
    ...
    void rediser::callerDel(const uint32 id, const char * key, const fRedisCall& call, bool sync, const iContext context) {
        if (sync) {
            autolocker(lock, _lock);
            checkContext();
            redisReply * res = (redisReply *)redisCommand(_context, "del %s", key);
    ...
    void rediser::doTransation(redisTransaction* tras, const uint32 id, const fRdsTransactionCall& call, bool sync, const iContext context) {
        if (sync) {
            tlib::cargs results;
    ...
    void rediser::onTimer(iCore * core, const int32 id, const iContext & context, const int64 tick) {
        switch (id) {
