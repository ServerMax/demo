...
namespace tcore {
    ...
    tcper::tcper(api::iTcpSession * session, const int32 sendsize, const int32 recvsize, uint32 socket)
        : _session(session),
        _send_buff(sendsize),
        _recv_buff(recvsize),
        _socket(socket),
    ...
    bool tcper::async_recv() {
        ...
        int32 res = WSARecv(_socket, &_recv_ex._wbuf, 1, &bytes, &flag, (LPWSAOVERLAPPED)&_recv_ex, nullptr);
    ...
    bool tcper::async_send() {
        ...
        int32 res = WSASend(_socket, &_send_ex._wbuf, 1, nullptr, 0, (LPWSAOVERLAPPED)&_send_ex, nullptr);
        int32 err = GetLastError();
    ...
    tcper * tcper::create(api::iTcpSession * session, const int32 sendsize, const int32 recvsize, uint32 sock, bool initiative) {
        ...
        if (initiative) {
            ...
            ex->_socket = sock;
            DWORD bytes = 0;
            int32 res = connectex(pipe->_socket, (struct sockaddr *)&ex->_remote, sizeof(struct sockaddr_in), nullptr, 0, &bytes, (LPOVERLAPPED)ex);
            int32 err = GetLastError();
    ...
    void tcper::onCompleter(overlappedex * ex, const eCompletion type, const int32 code, const int32 size) {
        ...
        case eCompletion::recved: {
            ...
            if (0 == code && 0 != size) {
                if (_recv_buff.in(_recv_temp, size)) {
                    ...
                    uint32 use = 0;
                    while (_recv_buff.getLength() > 0 && (use = _session->onRecv(core::getInstance(), (const char *)_recv_buff.getData(), _recv_buff.getLength())) > 0) {
                        _recv_buff.out(use);
    ...
    void tcper::send(const void * data, const int32 size, bool immediately) {
