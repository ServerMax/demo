...
namespace tcore {
    ...
    accepter * accepter::create(api::iTcpServer * server, const std::string & ip, const int32 port, const int32 ssize, const int32 rsize) {
    ...
    accepter::accepter(api::iTcpServer * server, const std::string & ip, const int32 port, const int32 s_size, const int32 r_size)
    : _server(server), _listen_addr(ip, port), _ssize(s_size), _rsize(r_size), _socket(INVALID_SOCKET), _ex(eCompletion::accepted, this) {
    }

    void accepter::onCompleter(overlappedex * ex, const eCompletion type, const int32 code, const int32 size) {
        ...
        if (0 == code) {
            ...
            if (SOCKET_ERROR == setsockopt(_ex._socket, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *)&_socket, sizeof(SOCKET))
                ...
                const int32 port = ntohs(sa.sin_port);
