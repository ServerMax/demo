...
namespace tcore {
    struct TimerContext : public iContext {
        const int32 _id;
        TimerContext(const int32 id, const iContext & context) : iContext(context), _id(id) {}
    ...
    bool timermgr::launch() {
        _timerGear[4] = tnew tgear(tq_tvn_size, 0);
        _timerGear[3] = tnew tgear(tq_tvn_size, _timerGear[4]);
        _timerGear[2] = tnew tgear(tq_tvn_size, _timerGear[3]);
        _timerGear[1] = tnew tgear(tq_tvn_size, _timerGear[2]);
        _timerGear[0] = tnew tgear(tq_tvr_size, _timerGear[1]);

        _running = tnew tlist;
        _suspended = tnew tlist;
        return true;
    }
    ...
    void timermgr::update(const int32 overtime) {
        static int64 last = tools::time::getMillisecond();
        int64 tick = tools::time::getMillisecond();

        int32 count = (int32)(tick - last) / jiffies_interval;
    ...
    tbase * timermgr::createTimerBase(iTimer * timer, const int32 id, const iContext & context, int32 count, int64 interval, const char * file, const int32 line) {
    ...
    void timermgr::removeTimerBase(tbase * base, iTimer * timer, const int32 id, const iContext & context) {
        TIME_MAP::iterator itor = s_timer_map.find(timer);
    ...
    tbase * timermgr::findTimerBase(iTimer * timer, const int32 id, const iContext & context) {
        TIME_MAP::iterator itor = s_timer_map.find(timer);
        if (itor != s_timer_map.end()) {
            TimerContext ct(id, context);
    ...
    void timermgr::startTimer(iTimer * timer, const int32 id, const iContext & context, int64 delay, int32 count, int64 interval, const char * file, const int32 line) {
    ...
    void timermgr::killTimer(iTimer * timer, const int32 id, const iContext & context) {
        tbase * base = findTimerBase(timer, id, context);
    ...
    void timermgr::pauseTimer(iTimer * timer, const int32 id, const iContext & context) {
        tbase * base = findTimerBase(timer, id, context);
        if (base == nullptr) {
    ...
    void timermgr::resumeTimer(iTimer * timer, const int32 id, const iContext & context) {
        tbase * base = findTimerBase(timer, id, context);
        if (base == nullptr) {
            //error(core::getInstance(), "where is timer base, timer %x, id %d", timer, id);
            return;
...
//         core::getInstance()->LogAsync("==============================trace timer end==========================", true);
    ...
    tlist* timermgr::findTimerList(uint64 expire) {
        uint64 live = expire - _jiffies;
        tlist* vec = 0;
        if (live < tq_tvr_size)
