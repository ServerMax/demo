...
namespace tools {
    ...
    namespace time {
        ...
        // second
        inline uint64 transformationTime(const std::string strtime, const char * format = "%4d-%02d-%02d %02d:%02d:%02d") {
        ...
        inline uint64 getWeekDay(const int64 tick) {
        ...
        inline std::string getTimeString(const int64 tick, const char * format = "%4d-%02d-%02d %02d:%02d:%02d"){
            char strtime[128];
            auto mTime = std::chrono::milliseconds(tick);
            auto tp = std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>(mTime);
            auto tt = std::chrono::system_clock::to_time_t(tp);
        ...
        inline std::string getTimeStringM_D_H_M(const int64 tick, const char * format = "%02d-%02d %02d:%02d") {
            char strtime[128];
            auto mTime = std::chrono::milliseconds(tick);
            auto tp = std::chrono::time_point<std::chrono::system_clock, std::chrono::milliseconds>(mTime);
            auto tt = std::chrono::system_clock::to_time_t(tp);
        ...
        inline std::string getQuantumTimeString(const int64 tick, const char * format = "%02d:%02d:%02d") {
            int h = tick / hour;
        ...
        inline std::string getMinuteSecondTimeString(const int64 tick, const char * format = "%02d:%02d") {
            int min = tick / minute;
    ...
    namespace file {
        ...
        bool getfiles(const char * dir, const char * ex, OUT opaths & paths, OUT onames & names, OUT int32 & count);
    }
    ...
    inline int32 split(const std::string & src, const std::string & commas, osplitres & res) {
        if (src == "") {
            return 0;
        }
    ...
    inline int32 stringAsInt(const char * value) {
    ...
    inline int64 stringAsInt64(const char * value) {
        tassert(value, "null point");
        return atoi64(value);
    }

    inline std::string int64AsString(const int64 value) {
        char str[128] = { 0 };
        safesprintf(str, sizeof(str), "%lld", value);
        return str;
    }

    inline std::string unsigned64AsString(const uint64 value) {
        char str[128] = { 0 };
        safesprintf(str, sizeof(str), "%llu", value);
        return str;
    }
    
    inline std::string intAsString(const int32 value) {
        char str[128] = { 0 };
    ...
    static uint64 static_seed = 0;
    inline void setSeed() {
        static_seed = time::getMicrosecond();
    }

    inline uint32 rand(uint32 range) {
        if (0 == range) { 
            return 0; 
        }
        if (static_seed == 0) {
...
inline std::string & operator<<(std::string & target, const int32 value) {
    target += tools::intAsString(value);
    return target;
}

inline std::string & operator<<(std::string & target, const int64 value) {
    target += tools::int64AsString(value);
    return target;
}

inline std::string & operator<<(std::string & target, const uint64 value) {
    target += tools::int64AsString(value);
    return target;
}
