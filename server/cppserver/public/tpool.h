...
namespace tlib {
    template <typename T, int32 chunk_count = 1, int32 chunk_size = 64>
    class tpool {
#ifdef _DEBUG
...
#ifdef _DEBUG
        T * create_object(const char * file, const int32 line) {
...
#ifdef _DEBUG
        T * create_object(const char * file, const int32 line, Args... args) {
            chunk * chunk = create(file, line);
#else
...
#ifdef _DEBUG
        void recover_object(T * t, const char * file, const int32 line) {
            //printf("recover %llx %s:%d\n", t, file, line);
            //_map.erase(_map.find(t));
#else
        void recover_object(T * t) {
#endif //_DEBUG
            t->~T();
            recover((chunk *)t);
        }

        int32 count() { return _chunk_count * chunk_size; }

    private:
#ifdef _DEBUG
        chunk * create(const char * file, const int32 line) {
#else
        chunk * create() {
#endif
            chunk * ret = nullptr;
        ...
        void alloc_chunk(int32 count) {
            for (int i = 0; i < count; ++i)
            {
                chunklist * new_list = (chunklist*)tmalloc(sizeof(chunklist));
...
#ifdef _DEBUG
#define create_from_pool(pool, ...) pool.create_object(__FILE__, __LINE__, ##__VA_ARGS__)
#define recover_to_pool(pool, p) pool.recover_object((p), __FILE__, __LINE__)
#else
#define create_from_pool(pool, ...) pool.create_object( __VA_ARGS__ )
#define recover_to_pool(pool, p) pool.recover_object(p)
#endif
#endif //__tpool_h__
