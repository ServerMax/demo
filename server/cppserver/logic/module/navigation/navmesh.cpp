...
int32 navmesh::findStraightPath(int32 layer, const geometry::vector3 & start, const geometry::vector3 & end, std::vector<geometry::vector3> & paths) const {
    if(layer >= (int32)_navmeshQuery_layers.size()) {
        return -1;
    }
    ...
    uint8 straightPathFlags[MAX_POLYS];
...
int32 navmesh::raycast(int32 layer, const geometry::vector3 & start, const geometry::vector3 & end, geometry::vector3 & hit) const {
    dtNavMeshQuery* navmeshQuery = _navmeshQuery_layers[layer];
...
bool navmesh::check(int32 layer, const geometry::vector3 & point) const {
    dtNavMeshQuery* navmeshQuery = _navmeshQuery_layers[layer];
