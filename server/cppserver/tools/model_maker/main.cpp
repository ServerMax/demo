...
static std::map<string, int32> s_index_map;
static std::map<string, int32> s_offset_map;
static std::map<string, string> s_table_map;
static std::map<string, int32> s_table_tag;


static std::map<string, string> s_ts_attr_map;
static std::map<string, int32> s_ts_index_map;
static std::map<string, int32> s_ts_offset_map;
static std::map<string, string> s_ts_table_map;
static std::map<string, int32> s_ts_table_tag;

string convertProp(const char * name, int32 & index, int32 & offset) {
    ...
    if (tinyxml2::XML_SUCCESS == xml.LoadFile(ifind->second.c_str())) {
        ...
        while (prop) {
            ...
            //static const attribute UserID(0, type_s64, sizeof(int64), false, false);
            std::string temp;
...
string convertPropTs(const char* name, int32& index, int32& offset) {
...
string ConvertTable(const char* name, int32& tag) {
    std::map<string, string>::iterator itor = s_table_map.find(name);
    if (itor != s_table_map.end()) {
        std::map<string, int32>::iterator itag = s_table_tag.find(name);
        tag = itag->second;
        return itor->second;
    }
    ...
    if (tinyxml2::XML_SUCCESS == xml.LoadFile(ifind->second.c_str())) {
        ...
        while (tab) {
            ...
            value += "                static const int32 tag = " + string(tools::intAsString(tag++).c_str()) + ";\n";
            value += "                enum {\n";
...
string ConvertTableTs(const char* name, int32& tag) {
    std::map<string, string>::iterator itor = s_ts_table_map.find(name);
    if (itor != s_ts_table_map.end()) {
        std::map<string, int32>::iterator itag = s_ts_table_tag.find(name);
        tag = itag->second;
        return itor->second;
    }
